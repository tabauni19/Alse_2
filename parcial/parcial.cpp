...
void leerPuntos(Point puntos[], int n)
{
    ...
    else
    {
        ...
        for (int i = 4; i < n; i++)
        {
            puntos[i] = {double(i * 3), double(i * 4)};
        }
    }
}
...
/*BONO:
Escribir una función que reciba un arreglo de puntos y devuelva la distancia total de los puntos
recorridos en orden.
*/
...
int main()
{
    ...
    // BONUS: mostrar la distancia total del recorrido
    double total = distanciaTotalRecorrido(puntos, n);
    std::cout << "La distancia total del recorrido entre los puntos es: " << total << std::endl;
